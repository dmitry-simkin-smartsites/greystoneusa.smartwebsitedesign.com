<?php
function add_pixel_blocker_filter_on_thankyou_page() {
    add_filter('pys_disable_server_event_filter', 'disable_pixel_on_thank_you_page', 10, 1);
} 

function disable_pixel_on_thank_you_page() {
    ...
    if ( is_order_received_page() && get_the_ID() === $id ) { 
        ...
        $show_thank_you = get_post_meta($order_id, '_show_thank_you', true);

        if ( 'true' === $show_thank_you ) {
            return true;  
        }

       
        if ( empty($show_thank_you) ) {
            update_post_meta($order_id, '_show_thank_you', '1'); 
            return false;  
        }
    }
}
       
add_action('template_redirect', 'add_pixel_blocker_filter_on_thankyou_page');
...
define('ESSENTIALS_THEME_VERSION', '3.1.5');
...
function pixfort_theme_scripts() {
	...
	wp_enqueue_script('pix-main-essentials', get_template_directory_uri() . '/js/essentials.min.js', array('jquery', 'jquery-ui-core', 'pix-bootstrap-js'), ESSENTIALS_THEME_VERSION, true);
	if (class_exists('WooCommerce') && pix_get_option('pix-old-popups')) {
	...
	if (pix_get_option('pix-exit-popup')) {
		if (pix_show_exit_popup()) {
			...
			$main_values['dataExitPopupOptionId'] = pix_get_option('pix-exit-popup-id');
		}
	}
	...
	// Bootstrap
	wp_enqueue_style('pixfort-bootstrap', get_template_directory_uri() . '/inc/scss/bootstrap.min.css', false, ESSENTIALS_THEME_VERSION, 'all');
...
function pixfort_theme_add_styles() {
	...
	if (is_user_logged_in()) wp_enqueue_style('pix-theme-admin-style', get_template_directory_uri() . '/css/pix-admin.css', false, ESSENTIALS_THEME_VERSION, 'all');
}
...
function pix_theme_footer_extras() {
	...
	wp_enqueue_style('pixfort-base-style', get_template_directory_uri() . '/css/base.min.css', false, ESSENTIALS_THEME_VERSION, 'all');
...
function pix_block_editor_styles() {
	...
	wp_enqueue_style('essentials-block-editor-styles', get_theme_file_uri('/css/editor-style-block.css'), false, ESSENTIALS_THEME_VERSION, 'all');
}
...
function pix_widget_display_callback($instance, $widget, $args) {
	if (empty($widget->widget_id) || $widget->widget_id !== 'woocommerce_widget_cart') {
		wp_enqueue_style('pixfort-widgets-style', get_template_directory_uri() . '/css/elements/widgets.min.css', false, ESSENTIALS_THEME_VERSION);
	}
	return $instance;
}
